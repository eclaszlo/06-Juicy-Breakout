...
onready var Chime = get_node("/root/Game/Chime")
onready var Coin = get_node("/root/Game/Coin")
onready var Hammer = get_node("/root/Game/Hammer")
...
func _physics_process(delta):
	...
	for body in bodies:
		...
		if body.is_in_group("Tiles"):
			...
			add_color(1.0)
			body.find_node("Sparkle").emitting = true
			Coin.playing = true
			body.kill()
		if body.name == "Paddle":
			body.find_node("Stars").emitting = true
			Chime.playing = true
